...
function msgbox(obj, width) {
    ...
    Ajax.request(url, {
        ...
        success: function (result) {
            closeline();
            his_mask(0);
            if (url.substr(12, 4) == "view") {
                result = "<div class=\"chat_log\">" + result + "</div>";
            }
            _("his_show", title, result, width, 0, 1);
        },
...
function fast(url, obj) {
    ...
    Ajax.json(url, {
        data: {req: Math.random()},
        success: function (result) {
            if (result.code == 1 && obj.substr(0, 3) == "del") {
                Del($("this_url").value);
            } else {
...
//快速更新 符合laravel
function fastUpdate(url, obj, form)
{
    ...
    Ajax.json(url, {
        ...
        success: function (result) {
            ...
            if (t == -1) {
                ...
                if (result.hasOwnProperty("field")) {
                    setTimeout('To("' + result.field + '","main");', 500);
                } else {
                    setTimeout('closeshow();', 500);
                }
            }
            if (t == -2) {
                m.innerHTML = icon + s;
                setvalue('key', result.field)
            }//生成密钥使用
            if (t == -3) {
                m.innerHTML = icon + s;
                if (result.hasOwnProperty("field")) {
                    setTimeout('window.location.href="/login/exit";', 2000)
                } else {
                    sb.disabled = false;
            ...
            if (t == -4) {
                m.innerHTML = icon + s;
                closeshow();
                To($("this_url").value, 'main');
            }//快速提交 返回
            if (t == -5) {
                m.innerHTML = icon + s;
                setTimeout('To($("this_url").value, "main");closeshow();', 500);
            }//MSBOX表格属性
        },
        ...
        failure: function (xhr, msg) {
            if (xhr.status == 422) {
                ...
                for (var key in ajaxobj.errors) {
                    sb.disabled = false;
                    m.innerHTML = icon + ajaxobj.errors[key];
                    focus($(key));//if(result.field.length>0){setTab(0);}
                    break;
                    // alert(errorList[key]);
                }
        ...
        Ajax.json(url, {
            success: function (result) {
                ...
                if (result.code == 0) {
                    ...
                    o.style.color = "red";
                    o.style.backgroundImage = "url(/img/cf.gif)";
...
function check_phone() {
    ...
    if (!istel.test(str)) {
        $('phone_city').innerHTML = "请正确填写电话号码";
        return false;
    }
...
function dbbak() {
    Ajax.json("call_back.asp", {
        ...
        success: function (result) {
            if (result.code == 0) {
                $("call_dbbak").innerHTML = result.msg;
            }
        }
    });
}
...
function track() {
    Ajax.json("call_back.asp", {
        ...
        success: function (result) {
            $("call_track").innerHTML = result.msg;
            if (result.code == 2) {
                alert("你有" + result.field + "条新的回拨需要处理！");
            }
            window.setTimeout("track()", 5120);
        }
    });
}
...
function weather() {
    Ajax.json("/weather", {
        ...
        success: function (result) {
            set_innerHTML("call_weather", result.msg);
            window.setTimeout("weather()", 3000000);
        }
...
function local(ip) {
    Ajax.json("call_back.asp", {
        ...
        success: function (result) {
            $("call_local").innerHTML = result.msg;
        }
    });
}
        ...
        success: function (result) {
            if (result.code == 0) {
                ...
                for (var name in result) {
                    ...
                    id = 'now_' + name;
                    if (result[name] > 0 && exist(id)) {
                        show(id);
                        $(id).innerHTML = result[name];
                    }
                }
    ...
    Ajax.json("call_back.asp", {
        ...
        success: function (result) {
            ...
            for (var i = 0; i < lists.length; i++) {
                id = 'now_' + lists[i];
                if (result[lists[i]] > 0 && exist(id)) {
                    show(id);
                    $(id).innerHTML = result[lists[i]];
                } else {
                    hide(id);
                }
            }
...
function fastT(n, u, t, b, c, d, e) {
    ...
    for (i = 0; i < menu.length; i++) {
        ...
        switch (u) {
            ...
            case 2    :
                ...
                break;
            case 3    :
                url = "/book/statistics/searchByMonth";
                break;
            case 4    :
                url = "/take/statistics/searchByMonth";
                break;
            case 5    :
                url = "/take/statistics/list";
                break;
            case 6    :
                url = "/dialog/statis/searchByMonth";
                break;
            case 7    :
                url = "/rank/statistics/searchByMonth";
                break;
    ...
    Ajax.request(url, {
        ...
        data: 'field=' + a + b + "&req=" + Math.random(),
        success: function (result) {
            close_mask('box');
            $("tablist").innerHTML = result;
        },
        failure: function (xhr, msg) {
            alert(msg)
        }
...
function showline(str) {
    if (!str) {
        _("his_line", 0, "请稍候...", 200, 40);
    } else {
        _("his_line", 0, str, 250, 40);
        setTimeout('closeline();', 1500);
    }
}
...
function _(obj, title, content, width, height, o) {
    ...
    if (obj == "his_line") {
        ...
        if (!height) {
            height = $("his_content").offsetHeight + 40;
        }
...
function getChange(o, m) {
    ...
    if (m == 1) {
        Par = o.parentNode.parentNode.getElementsByTagName("a");
    }
    var menu = $("sidebar").getElementsByTagName("ol");
...
function Copy(n) {
    window.clipboardData.setData("Text", $(n).value);
    alert("已复制,直接在QQ信息窗口右键粘贴发送即可！");
}
